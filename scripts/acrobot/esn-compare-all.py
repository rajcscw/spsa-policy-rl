...
for k in range(int(config["log"]["runs"])):
    ...
    for weight_selection in weight_selections:
        ...
        # do a run
        episodic_total_reward,_ = run_esn_policy_optimization_spsa(config, state_dim, weight_selection, "softmax")
    ...
    episodic_total_reward = run_actor_critic(config, "softmax")
